...
\begin{algorithm*}
	...
	\begin{algorithmic}
		...
		\State \texttt{allTickets}$\leftarrow$0
		\State \texttt{ticketList}$\leftarrow$\texttt{new LinkedList<Integer>}
		\State \texttt{threadList}$\leftarrow$\texttt{new LinkedList<ThreadState>}
		\For{all \texttt{ThreadState ts} in \texttt{priorityQueue}}
			\State \texttt{tsTicket}$\leftarrow$\texttt{ts.updateEffectivePriority}
			\State \texttt{allTickets}$\leftarrow$\texttt{allTickets}+\texttt{tsTicket}
			\State \texttt{ticketList.add(tsTicket)}
			\State \texttt{threadList.add(ts)}
		\EndFor
		\State \texttt{pickedTicket}$\leftarrow$\texttt{Random(allTickets)}
		\State \texttt{count}$\leftarrow$0
		\State \texttt{i}$\leftarrow$0
		\While{\texttt{count} less than \texttt{pickedTicket}}
			\State \texttt{count}$\leftarrow$\texttt{ticketList.get(i)+count}
			\State \texttt{i}$\leftarrow$\texttt{i+1}
		\EndWhile
		\State \Return \texttt{threadList.get(i)}
		\EndProcedure
...
\begin{algorithm*}
	...
	\begin{algorithmic}
		...
		\State \texttt{biasEP}$\leftarrow$0
		\For{all \texttt{PriorityQueue} \texttt{pq} in the \texttt{TreeSet<PriorityQueue>} \texttt{acquires}}
			\For{all \texttt{ThreadState} \texttt{ts} in \texttt{pq}}
				\texttt{biaEP} $\leftarrow$ \texttt{biaEP} + \texttt{ts.updateEffectivePriority}
			\EndFor
		\EndFor
		\State \texttt{waitingFor.occupyingThread.updateEffectivePriority}
		\EndProcedure
	\end{algorithmic}
\end{algorithm*}
\subsection{Test}
